...
draft: false
summary: 'How the Legal Framework is actually built in Claude Code: three tiers of agent context, three Python scripts, a reflective hook, and the operating loop. Includes the skill as a download, plus the parts that are not right yet.'
authors: ['default']
layout: PostLayout
---

I have been building an agent context system I call the Legal Framework: a Constitution for values, Legislation for the rules of each area, and a Common Law of accumulated experience recalled at the moment the rules run out. This post is how it is actually set up in Claude Code.
...
The idea is conceptual. The implementation is boringly concrete, and that is the point — each tier is a place on disk with its own loading behaviour.
...
The same goes for [what Anthropic have been posting](https://x.com/i/status/2087207428278395114) on this subject. Read it against the three tiers above and judge for yourself, but to my eye it lands remarkably close to where I got to independently. Convergence is not proof that an idea is right. It is decent evidence you are pointed at a real problem.
